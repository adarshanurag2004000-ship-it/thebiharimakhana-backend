...
app.post('/api/check-phone', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) { res.json({ isAvailable: false }); } 
...
app.get('/api/products', async (req, res) => {
    try {
        ...
        const queryParams = [];
        if (category) { queryParams.push(category); query += ` AND category = $${queryParams.length}`; }
        if (search) { queryParams.push(`%${search}%`); query += ` AND (name ILIKE $${queryParams.length} OR description ILIKE $${queryParams.length})`; }
...
app.get('/api/navigation', async (req, res) => {
    try {
        const { rows } = await pool.query('SELECT link_text, link_url FROM navigation_links ORDER BY sort_order ASC');
        res.json(rows);
    } catch (err) {
...
app.post('/api/apply-coupon', async (req, res) => {
    ...
    if (!cart || Object.keys(cart).length === 0) { return res.status(400).json({ success: false, message: 'Cart data is required.' }); }
    try {
        let subtotal = 0;
        for (const productName in cart) { subtotal += cart[productName].price * cart[productName].quantity; }
        ...
        if (couponCode) {
            ...
            if (couponResult.rows.length > 0) {
                ...
                if (coupon.discount_type === 'percentage') { discount = subtotal * (parseFloat(coupon.discount_value) / 100); } 
                else { discount = parseFloat(coupon.discount_value); }
                discount = Math.min(subtotal, discount);
            } else { return res.status(404).json({ success: false, message: "Invalid or inactive coupon code."}); }
        }
...
app.post('/checkout', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const query = `INSERT INTO orders (customer_name, phone_number, address, cart_items, order_amount, user_uid, razorpay_payment_id, coupon_used, discount_amount) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`;
        const values = [addressDetails.name, addressDetails.phone, addressDetails.address, JSON.stringify(cart), totalAmount, user.uid, paymentId, appliedCouponCode, discount];
        const orderResult = await pool.query(query, values);
        const newOrder = orderResult.rows[0];
        generateInvoicePdf(newOrder, (pdfData) => { sendOrderConfirmationEmail(user.email, addressDetails.name, newOrder, pdfData); });
...
app.post('/api/user-login', async (req, res) => {
    ...
    try {
        ...
        } else { console.log(`INFO: User ${email} already exists in database. Skipping insert.`); }
...
app.get('/api/my-orders', verifyToken, async (req, res) => {
    try {
        ...
        const { rows } = await pool.query('SELECT id, order_amount, created_at, cart_items, status FROM orders WHERE user_uid = $1 ORDER BY created_at DESC', [userUid]);
        res.json(rows);
    } catch (err) {
...
app.post('/api/request-deletion-code', verifyToken, async (req, res) => {
    ...
    try {
        await pool.query('UPDATE users SET delete_code = $1, delete_code_expires_at = $2 WHERE firebase_uid = $3', [code, expiresAt, uid]);
...
app.post('/api/verify-deletion', verifyToken, async (req, res) => {
    ...
    try {
        const { rows } = await pool.query('SELECT delete_code, delete_code_expires_at FROM users WHERE firebase_uid = $1', [uid]);
        if (rows.length === 0) { return res.status(404).json({ success: false, message: 'User not found.' }); }
        const user = rows[0];
        const now = new Date();
        if (user.delete_code !== code) { return res.status(400).json({ success: false, message: 'Invalid verification code.' }); }
        if (now > user.delete_code_expires_at) { return res.status(400).json({ success: false, message: 'Verification code has expired.' }); }
...
app.get('/api/products/:productName/reviews', async (req, res) => {
    try {
        ...
        const { rows } = await pool.query('SELECT rating, review_text, reviewer_name, created_at FROM reviews WHERE (product_name = $1 OR product_name = $2) AND is_approved = TRUE ORDER BY created_at DESC', [nameWithHyphen, nameWithSpace]);
        res.json(rows);
    } catch (err) {
...
app.post('/api/submit-review', verifyToken, async (req, res) => {
    ...
    if (!productName || !rating) { return res.status(400).json({ success: false, message: 'Product and rating are required.' }); }
    try {
        const userResult = await pool.query('SELECT is_blocked_from_reviewing FROM users WHERE firebase_uid = $1', [uid]);
        if (userResult.rows.length > 0 && userResult.rows[0].is_blocked_from_reviewing) { return res.status(403).json({ success: false, message: 'You are not permitted to leave reviews.' }); }
        ...
        const ordersResult = await pool.query(`SELECT id FROM orders WHERE user_uid = $1 AND (cart_items ->> $2 IS NOT NULL OR cart_items ->> $3 IS NOT NULL) AND status = 'Delivered'`, [uid, productNameWithHyphens, productNameWithSpaces]);
        if (ordersResult.rows.length === 0) { return res.status(403).json({ success: false, message: 'You can only review products you have purchased and received.' }); }
        const existingReview = await pool.query('SELECT id FROM reviews WHERE user_uid = $1 AND (product_name = $2 OR product_name = $3)', [uid, productNameWithHyphens, productNameWithSpaces]);
        if (existingReview.rows.length > 0) { return res.status(409).json({ success: false, message: 'You have already reviewed this product.' }); }
        await pool.query('INSERT INTO reviews (product_name, user_uid, rating, review_text, reviewer_name) VALUES ($1, $2, $3, $4, $5)', [productNameWithHyphens, uid, rating, reviewText, reviewerName]);
...
app.post('/api/my-addresses', verifyToken, async (req, res) => {
    ...
    if (!fullName || !phoneNumber || !street || !city || !pincode || !state || !country) { return res.status(400).json({ success: false, message: "All fields are required." }); }
    try {
        const { rows } = await pool.query(`INSERT INTO addresses (user_uid, full_name, phone_number, street, locality, city, pincode, state, country) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *`, [uid, fullName, phoneNumber, street, locality, city, pincode, state, country]);
...
app.delete('/api/my-addresses/:id', verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (result.rowCount === 0) { return res.status(404).json({ success: false, message: "Address not found or you don't have permission to delete it." }); }
...
app.get('/api/my-orders/:orderId/invoice', verifyToken, async (req, res) => {
    try {
        ...
        const { uid } = req.user;
        const { rows } = await pool.query('SELECT * FROM orders WHERE id = $1 AND user_uid = $2', [orderId, uid]);
        if (rows.length === 0) { return res.status(404).send('Order not found or you do not have permission to view it.'); }
        const order = rows[0];
...
app.post('/admin/login', (req, res) => {
    ...
    if (password === process.env.ADMIN_PASSWORD) {
        res.cookie('admin_session', process.env.ADMIN_SESSION_SECRET, { httpOnly: true, secure: process.env.NODE_ENV === 'production', maxAge: 24 * 60 * 60 * 1000 });
...
app.get('/admin/products', checkAdminAuth, async (req, res) => {
    try {
        ...
        const productsHtml = rows.map(p => {
            ...
            return `<tr><td>${p.id}</td><td><img src="${p.image_url}" alt="${he.encode(p.name)}"></td><td>${he.encode(p.name)}</td><td>${p.price}</td><td>${p.sale_price || 'N/A'}</td><td>${p.stock_quantity}</td><td>${he.encode(p.category || 'N/A')}</td><td>${featuredStatus}</td><td><a href="/admin/edit-product/${p.id}">Edit</a><form action="/admin/delete-product/${p.id}" method="POST" style="display:inline; margin-left: 5px;"><button type="submit" onclick="return confirm('Are you sure?');">Delete</button></form><form action="/admin/toggle-featured/${p.id}" method="POST" style="display:inline; margin-left: 5px;"><button type="submit">${toggleButtonText}</button></form></td></tr>`;
        }).join('');
...
app.post('/admin/toggle-featured/:id', checkAdminAuth, async (req, res) => {
    try {
        await pool.query('UPDATE products SET is_featured = NOT is_featured WHERE id = $1', [req.params.id]);
...
app.post('/admin/add-product', checkAdminAuth, async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO products(name, price, sale_price, stock_quantity, description, image_url, is_featured, category) VALUES($1, $2, $3, $4, $5, $6, $7, $8)', [value.productName, value.price, value.salePrice || null, value.stockQuantity, value.description, value.imageUrl, value.is_featured, value.category]);
        res.redirect(`/admin/products`);
...
app.post('/admin/update-product/:id', checkAdminAuth, async (req, res) => {
    ...
    if (error) { return res.status(400).send(error.details[0].message); }
    try {
        await pool.query('UPDATE products SET name = $1, price = $2, description = $3, image_url = $4, sale_price = $5, stock_quantity = $6, is_featured = $7, category = $8 WHERE id = $9', [value.productName, value.price, value.description, value.imageUrl, value.salePrice || null, value.stockQuantity, value.is_featured, value.category, id]);
        res.redirect(`/admin/products`);
...
app.post('/admin/delete-product/:id', checkAdminAuth, async (req, res) => {
    try { await pool.query('DELETE FROM products WHERE id = $1', [req.params.id]); res.redirect(`/admin/products`); } 
...
app.post('/admin/soft-delete-user/:uid', checkAdminAuth, async (req, res) => {
    try { await pool.query('UPDATE users SET deleted_at = NOW() WHERE firebase_uid = $1', [req.params.uid]); console.log(`Admin soft-deleted user ${req.params.uid}`); res.redirect('/admin/users'); } 
    catch (err) { console.error("Error soft-deleting user:", err); res.status(500).send('Error soft-deleting user.'); }
});
...
app.post('/admin/permanent-delete-user/:uid', checkAdminAuth, async (req, res) => {
    try {
        ...
        if (err.code === 'auth/user-not-found') {
            ...
            } catch (dbErr) { return res.status(500).send('Error permanently deleting user from database.'); }
        }
...
app.post('/admin/delete-order/:id', checkAdminAuth, async (req, res) => {
    try { await pool.query('DELETE FROM orders WHERE id = $1', [req.params.id]); console.log(`Admin deleted order ${req.params.id}`); res.redirect('/admin/orders'); } 
    catch (err) { console.error("Error deleting order:", err); res.status(500).send('Error deleting order.'); }
});
...
app.post('/admin/add-coupon', checkAdminAuth, async (req, res) => {
    ...
    try { await pool.query('INSERT INTO coupons(code, discount_type, discount_value) VALUES($1, $2, $3)', [code.toUpperCase(), discount_type, discount_value]); res.redirect('/admin/coupons'); } 
    catch (err) { res.status(500).send('Error adding coupon.'); }
});

app.post('/admin/delete-coupon/:id', checkAdminAuth, async (req, res) => {
    try { await pool.query('DELETE FROM coupons WHERE id = $1', [req.params.id]); res.redirect('/admin/coupons'); } 
    catch (err) { res.status(500).send('Error deleting coupon.'); }
});
...
app.post('/admin/delete-review/:id', checkAdminAuth, async (req, res) => {
    try { await pool.query('DELETE FROM reviews WHERE id = $1', [req.params.id]); res.redirect('/admin/reviews'); } 
    catch (err) { res.status(500).send('Error deleting review.'); }
});

app.post('/admin/block-user/:uid', checkAdminAuth, async (req, res) => {
    try { await pool.query('UPDATE users SET is_blocked_from_reviewing = TRUE WHERE firebase_uid = $1', [req.params.uid]); res.redirect('/admin/reviews'); } 
...
app.post('/admin/settings', checkAdminAuth, async (req, res) => {
    try {
        ...
        try {
            ...
            await client.query(`INSERT INTO site_settings (setting_key, setting_value) VALUES ('homepage_headline', $1) ON CONFLICT (setting_key) DO UPDATE SET setting_value = $1`, [homepage_headline]);
            await client.query(`INSERT INTO site_settings (setting_key, setting_value) VALUES ('homepage_subheadline', $1) ON CONFLICT (setting_key) DO UPDATE SET setting_value = $1`, [homepage_subheadline]);
            await client.query(`INSERT INTO site_settings (setting_key, setting_value) VALUES ('banner_text', $1) ON CONFLICT (setting_key) DO UPDATE SET setting_value = $1`, [banner_text]);
