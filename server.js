...
async function sendDeletionCodeEmail(customerEmail, code) {
    ...
    try { await sgMail.send(msg); console.log('Deletion code email sent to', customerEmail); } catch (error) { console.error('Error sending deletion code email:', error); }
}
async function sendOrderShippedEmail(customerEmail, customerName, orderId) {
    ...
    try { await sgMail.send(msg); console.log('Shipped notification email sent to', customerEmail); } catch (error) { console.error('Error sending shipped notification email:', error); }
}
...
app.post('/api/check-phone', async (req, res) => {
    ...
    } catch (err) { console.error('Error checking phone number:', err); res.status(500).json({ error: 'Internal server error.' }); }
});
...
app.get('/api/featured-products', async (req, res) => {
    try {
        ...
        res.json(rows);
    } catch (err) { console.error('Error fetching featured products:', err); res.status(500).send('Error fetching featured products'); }
});
    ...
    try {
        ...
        if (couponCode) {
            ...
            if (couponResult.rows.length > 0) {
                ...
                if (coupon.discount_type === 'percentage') { discount = subtotal * (parseFloat(coupon.discount_value) / 100); } else { discount = parseFloat(coupon.discount_value); }
...
app.post('/checkout', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error('Error during checkout:', err); res.status(500).json({ success: false, message: 'An internal server error occurred.' }); }
});
app.post('/api/user-login', async (req, res) => {
    ...
    } catch (err) { console.error("Error in /api/user-login:", err); res.status(500).json({ error: 'Internal server error.' }); }
});
app.get('/api/my-orders', verifyToken, async (req, res) => {
    try {
        ...
        res.json(rows);
    } catch (err) { console.error("Error fetching user orders:", err); res.status(500).send('Error fetching orders.'); }
});
app.post('/api/request-deletion-code', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error('Error requesting deletion code:', err); res.status(500).json({ success: false, message: 'Could not send deletion code.' }); }
});
app.post('/api/verify-deletion', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error(`Failed to verify and delete user ${uid}:`, err); res.status(500).json({ success: false, message: 'An error occurred during account deletion.' }); }
});
app.get('/api/products/:productName/reviews', async (req, res) => {
    try {
        ...
        res.json(rows);
    } catch (err) { console.error("Error fetching reviews:", err); res.status(500).send('Error fetching reviews.'); }
});
app.post('/api/submit-review', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error("Error submitting review:", err); res.status(500).json({ success: false, message: 'An error occurred while submitting your review.' }); }
});
app.get('/api/my-addresses', verifyToken, async (req, res) => {
    try {
        ...
        res.json(rows);
    } catch (err) { console.error("Error fetching addresses:", err); res.status(500).json({ success: false, message: 'Could not fetch addresses.' }); }
});
app.post('/api/my-addresses', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error("Error saving address:", err); res.status(500).json({ success: false, message: 'Could not save address.' }); }
});
app.delete('/api/my-addresses/:id', verifyToken, async (req, res) => {
    ...
    } catch (err) { console.error("Error deleting address:", err); res.status(500).json({ success: false, message: 'Could not delete address.' }); }
});
app.get('/api/active-coupons', async (req, res) => {
    try {
        ...
        res.json(rows);
    } catch (err) { console.error("Error fetching active coupons:", err); res.status(500).json([]); }
});
app.get('/api/my-orders/:orderId/invoice', verifyToken, async (req, res) => {
    try {
        ...
        const order = rows[0];
        generateInvoicePdf(order, (pdfData) => { res.setHeader('Content-Type', 'application/pdf'); res.setHeader('Content-Disposition', `attachment; filename="invoice-${order.id}.pdf"`); res.send(pdfData); });
    } catch (err) { console.error('Error generating invoice:', err); res.status(500).send('Error generating invoice.'); }
});// --- Admin Routes ---
